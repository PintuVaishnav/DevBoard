...
export default function Costs() {
  const { data: infraCosts, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) throw new Error('Failed to fetch infra costs');
      return res.json();
    },
  });

  if (isLoading || !infraCosts) {
    return (
  ...
  const { monthlyCosts, serviceCosts } = infraCosts;

  const totalCost = serviceCosts.reduce((sum: number, item: any) => sum + item.cost, 0);
  ...
  const costChange =
    lastMonthCost > 0 ? ((currentMonthCost - lastMonthCost) / lastMonthCost) * 100 : 0;

  const topService = serviceCosts.sort((a: any, b: any) => b.cost - a.cost)[0];
    ...
    <div className="space-y-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Monitor and analyze your cloud infrastructure expenses
        </p>
      </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Badge
                    className={`status-badge ${
                      costChange > 0 ? 'status-failed' : 'status-success'
                    }`}
                  >
